      ...
      <main className="flex-grow bg-secondary/20 py-8">
        <div className="container mx-auto px-4">
          ...
          <div className="space-y-6">
            {/* Pending Exchanges */}
            {exchanges.filter(ex => ex.status === "pending").length > 0 && <div>
                <h2 className="text-xl font-semibold mb-4">待处理的申请</h2>
                <div className="space-y-4">
                  {exchanges.filter(ex => ex.status === "pending").map(exchange => <div key={exchange.id} className="bg-white border border-border rounded-lg p-5">
            ...
            {/* Accepted Exchanges */}
            {exchanges.filter(ex => ex.status === "accepted").length > 0 && <div>
                <h2 className="text-xl font-semibold mb-4">进行中的交换</h2>
                <div className="space-y-4">
                  {exchanges.filter(ex => ex.status === "accepted").map(exchange => <div key={exchange.id} className="bg-white border border-border rounded-lg p-5">
                        <div className="flex justify-between items-start">
                          <div className="flex items-center">
                            <Avatar className="h-10 w-10 mr-3">
                              <AvatarImage src={exchange.otherUser.avatar} alt={exchange.otherUser.name} />
            ...
            {/* Completed Exchanges */}
            {exchanges.filter(ex => ex.status === "completed").length > 0 && <div>
                <h2 className="text-xl font-semibold mb-4">已完成的交换</h2>
                <div className="space-y-4">
                  {exchanges.filter(ex => ex.status === "completed").map(exchange => <div key={exchange.id} className="bg-white border border-border rounded-lg p-5">
                        <div className="flex justify-between items-start">
                          <div className="flex items-center">
                            <Avatar className="h-10 w-10 mr-3">
                              <AvatarImage src={exchange.otherUser.avatar} alt={exchange.otherUser.name} />
                        ...
                        {exchange.review && <div className="mt-4 p-3 bg-secondary/30 rounded-lg">
                            <div className="flex justify-between items-center mb-2">
                              <p className="text-sm font-medium">收到的评价：</p>
                              <div className="flex items-center">
                                {Array.from({
                        length: 5
                      }).map((_, i) => <Star key={i} className={`h-4 w-4 ${i < exchange.review!.rating ? "text-yellow-500 fill-yellow-500" : "text-gray-300"}`} />)}
                              </div>
                            </div>
            ...
            {exchanges.length === 0 && <div className="bg-white border border-border rounded-lg p-8 text-center">
